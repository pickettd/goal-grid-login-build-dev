...
.directive('flowChart', function() {
	return {
		restrict: 'E',
		templateUrl: "/static/bower_components/AngularJS-FlowChart/flowchart/flowchart_template.html",
		replace: true,
		scope: {
			chart: "=chart",
			rearrangeNodes: '=rearrangeNodes',
			nodeClickCallback: '=nodeClickCallback',
			dragEnabled: '=dragEnabled',
			emptySpaceClickCallback: '=emptySpaceClickCallback'
		},
	...
	*/
	...
	$scope.mouseDown = function (evt) {
		...
		if (!$scope.dragEnabled) {
			evt.preventDefault();
			var isMouseOverElement = controller.hitTest(evt.clientX, evt.clientY);
			var checkScope = controller.checkForHit(isMouseOverElement, controller.nodeClass);
			var isMouseOverNode = (checkScope && checkScope.node) ? checkScope.node : null;
			if (isMouseOverNode === null) {
				$scope.emptySpaceClickCallback(evt);
			}
			return;
		}
	...
	$scope.nodeMouseDown = function (evt, node) {
		...
		var lastMouseCoords;
		if (!$scope.rearrangeNodes) {
			evt.preventDefault();
			evt.stopPropagation();
			$scope.nodeClickCallback(node.data.id);
			return;
		}
