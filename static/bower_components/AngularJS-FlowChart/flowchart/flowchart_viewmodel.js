...
(function () {
	...
	//
	// Default callback when a new connection is made.
	//
	flowchart.defaultNewConnectionCallback = angular.noop;

	//
	// Default callback when a connection is removed.
	//
	flowchart.defaultRemoveConnectionCallback = angular.noop;

	//
	// Default callback when a node is removed.
	//
	flowchart.defaultRemoveNodeCallback = angular.noop;

	//
	// Default display options
	//
	flowchart.defaultShowGridLines = false;

	flowchart.defaultShowGridHeaders = false;
	flowchart.defaultHeaderElementArray = [];

	flowchart.defaultColorCodeByLinkPath = false;
	...
	flowchart.ChartViewModel = function (chartDataModel) {
		...
		// Set gridlines option
		this.showGridLines = chartDataModel.showGridLines || flowchart.defaultShowGridLines;

		// Set headers option
		this.showGridHeaders = chartDataModel.showGridHeaders || flowchart.defaultShowGridHeaders;

		this.headerElementArray = chartDataModel.headerElementArray || flowchart.defaultHeaderElementArray;

		// Set link color coding option
		this.colorCodeByLinkPath = chartDataModel.colorCodeByLinkPath || flowchart.defaultColorCodeByLinkPath;

		// Set new connection callback
		this.newConnectionCallback = chartDataModel.newConnectionCallback || flowchart.defaultNewConnectionCallback;

		// Set remove connection callback
		this.removeConnectionCallback = chartDataModel.removeConnectionCallback || flowchart.defaultRemoveConnectionCallback;

		// Set remove node callback
		this.removeNodeCallback = chartDataModel.removeNodeCallback || flowchart.defaultRemoveNodeCallback;
		...
		this.createNewConnection = function (startConnector, endConnector) {
			...
			this.newConnectionCallback(connectionDataModel);
		};
		...
		this.deleteSelected = function () {
			...
			for (var connectionIndex = 0; connectionIndex < this.connections.length; ++connectionIndex) {
				...
				// If this connection is going to be removed, fire the connection removal callback
				else {
					this.removeConnectionCallback(connection.data);
				}
			}

			for(var deletedNodeIndex = 0; deletedNodeIndex < deletedNodeIds.length; deletedNodeIndex++) {
				var nodeId = deletedNodeIds[deletedNodeIndex];
				this.removeNodeCallback(nodeId);
			};
