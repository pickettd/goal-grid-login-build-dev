      ...
      promise.then(function () {
        ...
        if (packeryEls.indexOf(el[0]) === -1) {
          imagesLoaded(el, function() {
            if (self.stampAttrib) {
              if (self.stamps === undefined) {
                self.stamps = angular.element(self.stampAttrib);
              }
              self.packery.stamp(self.stamps);
            }
  ...
  var packeryDirective = function (config, service) {
    ...
    return {
      ...
      link: function (scope, element, attrs, controller) {
        ...
        scope.stamp = scope.stamp || config.stamp;
        ...
        if (packery instanceof Packery) {
          ...
          if (scope.stamp !== '') {
            controller.stampAttrib = scope.stamp;
          }
